...
function Default(;
	...
	return Default(pars, bgrid, ygrid, Py, v, vR, vD, prob, gc, gb, q, qD, α, ι )
end
...
function budget_constraint(bpv, bv, yv, q, dd::Deuda)
	...
	cv = yv + q * (bpv - (1-ρ) * bv) - κ*(1 + dd.α*(yv - mean(dd.ygrid))) * bv
	return cv
end
...
function eval_value(jb, jy, bpv, itp_q, itp_v, dd::Deuda)
	...
	if dd.ι != 0
		for (jyp, ypv) in enumerate(dd.ygrid)
			prob = dd.Py[jy, jyp]
			Ev += prob *exp(-itp_v(bpv, ypv)*dd.ι)
		end
		Ev = (-1/ dd.ι)*log(Ev)
	else
		for (jyp, ypv) in enumerate(dd.ygrid)
...
function mpe!(dd::Deuda; tol = 1e-8, maxiter = 500,verbose =false)
	new_v = similar(dd.v)
	new_q = similar(dd.q)
	...
	while dist > tol && iter < maxiter
		...
		verbose && print("dist (v,q) = ($(round(dist_v, sigdigits=2)), $(round(dist_q, sigdigits=2)))\n")
	end
end
