...
function IFP(;β=0.96,θ=0, r=0.02, γ=2, Nk = 20, Ny = 25, μy = 1, ρy = 0.8, σy = 0.02,k =1)
	kgrid = range(0,k,length=Nk)
	...
	return IFP(β, γ, r, kgrid, ygrid, Py, v, gc, gk,θ)
end
...
function eval_value(kpv, kv, yv, py, itp_v::AbstractInterpolation, ce::IFP)
	...
	if ce.θ != 0
		for (jyp, ypv) in enumerate(ce.ygrid)
			prob = py[jyp]
			Ev += prob *exp(-itp_v(kpv, ypv)*ce.θ)
		end
		Ev = (-1/ ce.θ)*log(Ev)
	else
		for (jyp, ypv) in enumerate(ce.ygrid)
